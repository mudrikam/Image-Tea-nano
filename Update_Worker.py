...
def get_fallback_version():
    config = load_app_config()
    version = config.get("fallback_version", config.get("version", "1.0.97"))
...
class UpdateWorkerDialog(QDialog):
    ...
    def _on_update_success(self):
        ...
        reply = QMessageBox.question(
            self,
            "Update Complete",
            "Image Tea has been updated successfully!\n\nDo you want to relaunch the application now?\n\nYes: restore and merge configs from the pre-update backup (user settings will be preserved).\nNo: skip restore and keep your current configuration (you can restore later from the Backups dialog).",
