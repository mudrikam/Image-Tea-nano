...
class PromptSectionWidget(QWidget):
    def __init__(self, parent=None):
        ...
        max_retries_group = QVBoxLayout()
        max_retries_group.setSpacing(2)
        max_retries_group.setContentsMargins(0, 0, 0, 0)
        max_retries_header = QHBoxLayout()
        max_retries_header.setSpacing(3)
        max_retries_header.setContentsMargins(0, 0, 0, 0)
        max_retries_icon = QLabel()
        max_retries_icon.setPixmap(qta.icon('fa6s.rotate-right', color=icon_color).pixmap(icon_size, icon_size))
        max_retries_label = QLabel("Max Retry")
        max_retries_label.setStyleSheet(f"color: {theme.get_color('gray')}; font-size: 10px;")
        max_retries_header.addWidget(max_retries_icon)
        max_retries_header.addWidget(max_retries_label)
        max_retries_header.setAlignment(Qt.AlignmentFlag.AlignLeft)
        max_retries_group.addLayout(max_retries_header)
        self.max_retries_spin = QSpinBox()
        self.max_retries_spin.setRange(0, 10)
        self.max_retries_spin.setFixedWidth(fixed_width)
        self.max_retries_spin.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        self.max_retries_spin.setToolTip("Max retry attempts for failed files per batch.\n0 = no retry. Retries only the failed files, not the whole batch.")
        max_retries_group.addWidget(self.max_retries_spin)

        max_retries_wrapper = QWidget()
        max_retries_wrapper.setLayout(max_retries_group)
        max_retries_wrapper.setFixedWidth(fixed_width)
        main_layout.addWidget(max_retries_wrapper)
        ...
        self.max_retries_spin.valueChanged.connect(self.save_prompt_config)
    ...
    def load_prompt_config(self):
        ...
        try:
            ...
            self.max_retries_spin.setValue(data["max_retries"])
    ...
    def save_prompt_config(self):
        ...
        data["max_retries"] = self.max_retries_spin.value()
