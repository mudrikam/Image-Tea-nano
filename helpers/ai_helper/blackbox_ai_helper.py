...
def track_blackbox_generation_time(duration_ms):
    _generation_times_blackbox.append(duration_ms)
    if len(_generation_times_blackbox) > 1000:
        _generation_times_blackbox.pop(0)
    gen_time = duration_ms
    avg_time = int(sum(_generation_times_blackbox) / len(_generation_times_blackbox)) if _generation_times_blackbox else 0
    longest_time = max(_generation_times_blackbox) if _generation_times_blackbox else 0
    last_time = _generation_times_blackbox[-1] if _generation_times_blackbox else 0
    return gen_time, avg_time, longest_time, last_time

def load_blackbox_prompt_vars():
...
def format_blackbox_prompt(
    ...
    print("="*80)
    print("BLACKBOX FULL PROMPT (JSON FORMAT):")
    print("="*80)
    print(full_prompt)
    print("="*80)
...
def generate_metadata_blackbox(api_key, model, image_path, prompt=None, stop_flag=None, provider_endpoint=None, preextracted_frames=None, metadata_context=None, generate_prompt=False):
    ...
    try:
        ...
        if not prompt:
            (
                ...
                shutterstock_video_map,
                adobe_map
            ) = load_blackbox_prompt_vars()
            prompt = format_blackbox_prompt(
                title_requirements,
                description_requirements,
                keywords_requirements,
                general_guides,
        ...
        if provider_endpoint:
            ...
            try:
                ...
                text = CustomEndpointHelper.call_endpoint(api_key, provider_endpoint, 'blackbox', model, prompt, endpoint_image_path, timeout=180)
        ...
        try:
            ...
            print(f"[Blackbox JSON PARSE ERROR] {e}")
            title = description = tags = ''
            category = {}
            filetype = ''
            file_prompt = ''
            error_message = f"[Blackbox JSON PARSE ERROR] {e}"
        ...
        err_str = str(e)
        error_message = f"[Blackbox ERROR] {err_str}"
        print(error_message)
        code = None
        status = None
        try:
            ...
            print(f"[Blackbox] Error extracting code/status from exception: {e2}")
        try:
            if code or ('quota' in err_str.lower()) or ('rate limit' in err_str.lower()):
                signature = str(code) if not status else f"{code}|{status}"
                print(f"[Blackbox] Emitting error dialog: code={code}, signature={signature}, service=blackbox, file={os.path.basename(image_path)}")
                try:
                    from dialogs.ai_helper_error_code_dialog import invoker
                    invoker.showRequested.emit(signature, err_str, os.path.basename(image_path), 'blackbox')
                    ...
                    print(f"[Blackbox] Error dialog emission successful")
                except Exception as e_dialog:
                    print(f"[Dialog Error] Failed to show error dialog: {e_dialog}")
                    traceback.print_exc()
        except Exception as e2:
            print(f"[Blackbox] Error during error-dialog notification: {e2}")
            traceback.print_exc()
        return '', '', '', {}, '', error_message, 0, 0, 0, ''
    finally:
        duration_ms = int((time.perf_counter() - start_time) * 1000)
        track_blackbox_generation_time(duration_ms)
