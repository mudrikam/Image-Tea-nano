...
class IllustratorJSXGenerator:
    ...
    def generate_resident_jsx(self):
        ...
        jsx.append("            exportWithContentArtboard(doc, exportFile, pngOptions, 'PNG');")
        ...
        jsx.append("            exportWithContentArtboard(doc, exportFile, jpgOptions, 'JPG');")
        ...
        jsx.append("            saveWithContentArtboard(doc, exportFile, aiOptions, 'AI');")
        ...
        jsx.append("            saveWithContentArtboard(doc, exportFile, epsOptions, 'EPS');")
        ...
        jsx.append("            saveWithContentArtboard(doc, exportFile, pdfOptions, 'PDF');")
        ...
        jsx.append("            exportWithContentArtboard(doc, exportFile, svgOptions, 'SVG');")
        jsx.append("        }")
        ...
        jsx.append("}")
        jsx.append("")
        jsx.append("function getContentArtboard(doc) {")
        jsx.append("    var bounds = null;")
        jsx.append("    try {")
        jsx.append("        app.executeMenuCommand('deselectall');")
        jsx.append("        app.executeMenuCommand('selectall');")
        jsx.append("        for (var i = 0; i < doc.selection.length; i++) {")
        jsx.append("            var itemBounds = doc.selection[i].visibleBounds;")
        jsx.append("            if (!bounds) bounds = [itemBounds[0], itemBounds[1], itemBounds[2], itemBounds[3]];")
        jsx.append("            else {")
        jsx.append("                bounds[0] = Math.min(bounds[0], itemBounds[0]);")
        jsx.append("                bounds[1] = Math.max(bounds[1], itemBounds[1]);")
        jsx.append("                bounds[2] = Math.max(bounds[2], itemBounds[2]);")
        jsx.append("                bounds[3] = Math.min(bounds[3], itemBounds[3]);")
        jsx.append("            }")
        jsx.append("        }")
        jsx.append("    } finally {")
        jsx.append("        app.executeMenuCommand('deselectall');")
        jsx.append("    }")
        jsx.append("    if (!bounds || bounds[2] <= bounds[0] || bounds[1] <= bounds[3]) return null;")
        jsx.append("    var padding = 0.01;")
        jsx.append("    bounds = [bounds[0] - padding, bounds[1] + padding, bounds[2] + padding, bounds[3] - padding];")
        jsx.append("    return doc.artboards.add(bounds);")
        jsx.append("}")
        jsx.append("")
        jsx.append("function configureContentArtboardOptions(options, format, artboardIndex) {")
        jsx.append("    if (format == 'PNG' || format == 'JPG') {")
        jsx.append("        options.artBoardClipping = true;")
        jsx.append("        options.artBoardRange = String(artboardIndex + 1);")
        jsx.append("    } else if (format == 'SVG') {")
        jsx.append("        options.artboardClipping = true;")
        jsx.append("        options.artboardRange = String(artboardIndex + 1);")
        jsx.append("    } else if (format == 'PDF' || format == 'EPS') {")
        jsx.append("        options.saveMultipleArtboards = true;")
        jsx.append("        options.artboardRange = String(artboardIndex + 1);")
        jsx.append("    } else if (format == 'AI') {")
        jsx.append("        options.saveMultipleArtboards = true;")
        jsx.append("        options.artboardRange = String(artboardIndex + 1);")
        ...
        jsx.append("}")
        jsx.append("")
        jsx.append("function exportWithContentArtboard(doc, exportFile, options, format) {")
        jsx.append("    var artboard = getContentArtboard(doc);")
        jsx.append("    var artboardIndex = artboard ? doc.artboards.length - 1 : doc.artboards.getActiveArtboardIndex();")
        jsx.append("    if (artboard) doc.artboards.setActiveArtboardIndex(artboardIndex);")
        jsx.append("    configureContentArtboardOptions(options, format, artboardIndex);")
        jsx.append("    if (format == 'PNG') doc.exportFile(exportFile, ExportType.PNG24, options);")
        jsx.append("    else if (format == 'JPG') doc.exportFile(exportFile, ExportType.JPEG, options);")
        jsx.append("    else if (format == 'SVG') doc.exportFile(exportFile, ExportType.SVG, options);")
        jsx.append("}")
        jsx.append("")
        jsx.append("function saveWithContentArtboard(doc, exportFile, options, format) {")
        jsx.append("    var artboard = getContentArtboard(doc);")
        jsx.append("    var artboardIndex = artboard ? doc.artboards.length - 1 : doc.artboards.getActiveArtboardIndex();")
        jsx.append("    if (artboard) doc.artboards.setActiveArtboardIndex(artboardIndex);")
        jsx.append("    configureContentArtboardOptions(options, format, artboardIndex);")
        jsx.append("    if (format == 'AI') doc.saveAs(exportFile, options);")
        jsx.append("    else if (format == 'EPS') doc.saveAs(exportFile, options);")
        jsx.append("    else if (format == 'PDF') doc.saveAs(exportFile, options);")
        jsx.append("}")
        jsx.append("")
