...
class CreditUsageDialog(QDialog):
    def __init__(self, api_key: str, parent=None):
        ...
        sep.setStyleSheet(f"color: {theme.get_color('background_dark')};")
    ...
    def _populate_content(self, info):
        ...
        of_lbl = QLabel(f"of ${max_budget:.4f}  ·  ${remain:.4f} left  ·  {budget_pct:.1f}% used")
        ...
        budget_vbox.addWidget(self._make_row("Duration", budget_duration if budget_duration else 'None'))
        ...
        auto_rotate = info.get('auto_rotate', False)
        rotate_sep = QLabel("·")
        rotate_sep.setStyleSheet(f"font-size: 11px; opacity: 0.6;")
        rotate_lbl = QLabel(f"Auto Rotate: {auto_rotate}")
        rotate_lbl.setStyleSheet(f"font-size: 11px; opacity: 0.6;")
        status_val_row.addWidget(rotate_sep)
        status_val_row.addWidget(rotate_lbl)
        ...
        key_vbox.addWidget(self._make_row("Key Name", key_name, bold=True))
        key_vbox.addWidget(self._make_row("Key Alias", key_alias))
        key_vbox.addWidget(self._make_row("User ID", info.get('user_id') or '-'))
        key_vbox.addWidget(self._make_row("Team ID", info.get('team_id') or '-'))
