...
OpenRouter dapat digunakan sebagai alternatif penyedia API dan bersifat kompatibel dengan API OpenAI.

1. Dapatkan API key dari OpenRouter di `https://openrouter.ai/settings/keys` dan salin API key-nya.

Untuk panduan konfigurasi OpenRouter di Image Tea (cara menambahkan model lewat Model Manager dan mendaftarkan API key), lihat panduan Menambahkan API Key.

Catatan: OpenRouter kompatibel dengan OpenAI; Image Tea menggunakan entri `OpenAI` saat mengonfigurasi OpenRouter agar permintaan diarahkan dengan benar.
