...
5. To add OpenRouter in Image Tea (OpenRouter is OpenAI‑compatible):
	...
	a. In the **Add API Key** dialog, click the gear icon next to the `Model` dropdown to open **Model Manager**. Click `+` to add the model ids you need (for example `gpt-4o`, `gpt-5-mini`, or `openai/gpt-4.1-nano`) and save them.
